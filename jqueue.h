...
private:
    ...
    void copyHeap(Node *copied, Node* otherTree);

    //helper function to print heap in preorder fashion
    void preorder(Node* root) const;

    //helper function to print priorties in inorder traversal
    void inorder(Node* root) const;

    //helper function to merge two skew heaps
    Node* merge(Node* heap1, Node* heap2);

    //helper function checks if JQueue is empty
    bool empty();
